            ...
            foreach (var record in records)
            {
                ...
                foreach (var pair in record.Select(s => new { Key = s.Key.FieldName, Value = s.Value }))
                {
                    ...
                    if (property != null)
                    {
                        ...
                        if (property.PropertyType.Name.Equals("String")&&!property.ToString().Contains("DSW_JOB"))
                        {
                            if (property.PropertyType == pair.Value.GetType())
                            {
                                property.SetValue(item, ConvertTo.UnicodeFrom(TextEncoding.Arabic1256, pair.Value.ToString()), null);
